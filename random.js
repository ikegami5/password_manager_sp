...
export class Random {
    ...
    constructor(seed) {
        ...
        this.seed = new Binary(seed.toString(2)).xor(this.multiplier).and(this.mask);
    }

    next(bits) {
        this.seed = this.seed.multiply(this.multiplier).sum(new Binary('1011')).and(this.mask);
    ...
    nextInt(bound) {
        ...
        if ((bound & -bound) === bound) {
            ...
            return parseInt(this.next(31).right_shift(31 - bound_length + 1).value, 2);
        }
        let bits;
        let val;
        do {
