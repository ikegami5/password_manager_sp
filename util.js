...
async function read_file() {
    return await RNFS.readDir(RNFS.ExternalDirectoryPath).then(files => {
...
function get_password(salt, master_password, password_length, has_capital, has_numeral, symbols) {
    const hashed_string = hash_64_times(master_password, salt);
    return format_password(hashed_string, password_length, has_capital, has_numeral, symbols);
}

export function format_password(str, password_length, has_capital, has_numeral, symbols) {
    const initial_seed = parseInt(str.substring(456, 484), 2) + parseInt(str.substring(484), 2);
    ...
    for (let additional_seed = 0; additional_seed <= 100; additional_seed++) {
        ...
        const tmp_password = [...Array(password_length).keys()].map((index) => {
            const word_length = Math.floor(str.length / password_length);
            const word_string = str.substring(word_length * index, word_length * index + word_length - 1);
            const letter_index = new Binary(word_string).mod(new Binary(tmp_letters_list.length.toString(2)));
        ...
        const is_proper_password =
            ...
            (symbols ? new RegExp('^.*[' + symbols.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '].*$').test(tmp_password) : true);
...
export function hash(str) {
    ...
    const binary = Array.prototype.map.call(hex, (c) => {
